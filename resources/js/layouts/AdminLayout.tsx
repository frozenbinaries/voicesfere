...
export default function AdminLayout({ children, title }: AdminLayoutProps) {
    ...
    const navigation = [
        { name: 'Dashboard', href: '/dashboard', icon: LayoutDashboard, current: isRouteActive('/dashboard') },
        { name: 'Elections', href: '/elections', icon: Vote, current: isRouteActive('/elections') },
        { name: 'Voters', href: '/voters', icon: Users, current: isRouteActive('/voters') },
        { name: 'Results', href: '/results', icon: BarChart3, current: isRouteActive('/results') },
        { name: 'Calendar', href: '/calendar', icon: Calendar, current: isRouteActive('/calendar') },
        { name: 'Settings', href: '/settings', icon: Settings, current: isRouteActive('/settings') },
    ];
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-[#FDFDFC] via-white to-red-50 dark:from-[#0a0a0a] dark:via-[#0f0f0f] dark:to-red-950/20">
            ...
            {/* Sidebar for desktop */}
            <aside
                className={`fixed left-0 top-0 z-40 h-full bg-white shadow-xl transition-all duration-300 dark:bg-[#161615] ${
            ...
            >
                ...
                {/* Main navigation */}
                <nav className="mt-6 flex flex-1 flex-col justify-between h-[calc(100vh-4rem)]">
                    <div className="space-y-1 px-2">
                        {navigation.map((item) => (
                            ...
                            >
                                <item.icon className={`h-5 w-5 ${!sidebarOpen && 'mr-0'} ${sidebarOpen && 'mr-3'} ${item.current ? 'text-white' : 'text-gray-400 group-hover:text-red-600'}`} />
                    ...
                    <div className="mb-6 space-y-1 px-2">
                        <div className={`border-t border-[#e3e3e0] dark:border-[#3E3E3A] ${sidebarOpen ? 'mx-2' : 'mx-0'} my-2`}></div>
                        {secondaryNavigation.map((item) => (
                            ...
                            >
                                <item.icon className={`h-5 w-5 ${!sidebarOpen && 'mr-0'} ${sidebarOpen && 'mr-3'} text-gray-400 group-hover:text-red-600`} />
                                {sidebarOpen && <span>{item.name}</span>}
                            </Link>
                        ))}
                        ...
                        {/* User section */}
                        <div className={`border-t border-[#e3e3e0] dark:border-[#3E3E3A] ${sidebarOpen ? 'mx-2' : 'mx-0'} my-2`}></div>
                        <div className={`flex items-center ${sidebarOpen ? 'px-3 py-2' : 'justify-center py-2'}`}>
                        ...
                        >
                            <LogOut className={`h-5 w-5 ${!sidebarOpen && 'mr-0'} ${sidebarOpen && 'mr-3'}`} />
            ...
            {mobileMenuOpen && (
                <div className="fixed inset-0 z-50 md:hidden">
                    ...
                    <div className="fixed left-0 top-0 h-full w-64 bg-white shadow-xl dark:bg-[#161615]">
                        <div className="flex h-16 items-center justify-between border-b border-[#e3e3e0] px-4 dark:border-[#3E3E3A]">
                            <div className="flex items-center space-x-2">
                                <div className="h-8 w-8 rounded-full bg-red-600"></div>
                                <span className="text-xl font-bold text-[#1b1b18] dark:text-white">
                        ...
                        <nav className="mt-6 flex flex-1 flex-col justify-between h-[calc(100vh-4rem)]">
                            ...
                            <div className="mb-6 space-y-1 px-2">
                                ...
                                <div className="border-t border-[#e3e3e0] dark:border-[#3E3E3A] my-2"></div>
                ...
                <header className="sticky top-0 z-30 border-b border-[#e3e3e0] bg-white/80 backdrop-blur-md dark:border-[#3E3E3A] dark:bg-black/80">
                    <div className="flex items-center justify-between px-4 py-3 md:px-6">
                        ...
                        <div className="flex items-center space-x-3">
                            ...
                            {/* User dropdown */}
                            <div className="flex items-center space-x-3">
                                <div className="h-8 w-8 rounded-full bg-red-100 text-red-600 flex items-center justify-center dark:bg-red-900/30">
                                    <User className="h-4 w-4" />
                                </div>
